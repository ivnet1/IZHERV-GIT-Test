...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun){
            ...
            float angleStep = (shotgunBullets > 1) ? (shotgunSpread / (shotgunBullets - 1)) : 0;
            for (int i = 0; i < shotgunBullets; i++){
                float currentAngle = startAngle + (angleStep * i);
                Quaternion spreadRotation = Quaternion.Euler(0.0f, 0.0f, currentAngle);
                Quaternion finalRotation = director.rotation * spreadRotation;
                SpawnBullet(spawnPosition, finalRotation);
            }
        }else{
